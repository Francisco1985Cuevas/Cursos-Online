<?php
	...
	$db->connectdb($host, $user, $pass, $database);	//Se lleva a cabo la conexion a la base de datos...

	$query = $db->query("SELECT id, comunidad FROM comunidades");
	while($data = $db->fetch_array($query)):
		//Guarda los nombres de las comunidades...
		$listaComunidades .= '<option value="'.$data['id'].'">'.$data['comunidad'].'</option>';
	endwhile;

	for($x = 2003; $x < 2009; $x++):
		//Guarda los anhos de las comunidades...
