							...
							<td>
								<xsl:choose>
									<xsl:when test="on_promotion &gt; 0">
										<input type="checkbox" name="on_promotion" disabled="disabled" checked="checked"/>
									</xsl:when>
									<xsl:otherwise>
		...
		<xsl:template name="menu">
			<xsl:for-each select="data/params">
				<table>
					<tr>
						<td class="left">
							<xsl:value-of select="items_count" /> Películas
						</td> 
						<td class="right"> 
							<xsl:choose>
						...
						<td class="right">
							página <xsl:value-of select="returned_page" />
